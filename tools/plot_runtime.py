...
for index, exp_name in enumerate(exp_names):
    ...
    #with open("./experiment_plots/annotation_time.txt", "a") as out:
    #    out.write(exp_name+'\n')
...
plt.legend(bbox_to_anchor=(-0.8, -0.55), loc="lower center", ncol = 3,markerscale=12)
fig.subplots_adjust(left=0.09, right=0.97, bottom=0.09, top=0.96, hspace = 0.4, wspace = 0.3)
plt.savefig("./experiment_plots/runtime.pdf", dpi = 400)
plt.show()
